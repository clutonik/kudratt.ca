...
from fastapi import File, UploadFile, Request, Form, APIRouter
...
logging.basicConfig(filename='myrouter.log', level=logging.INFO)
...
async def create_upload_files(product:str= Form(...),description:str= Form(...),files: List[UploadFile] = File(...) ):
    ...
    try:
        ...
        for file in files:
            try:
                response =s3_client.sync_upload_fileobj(file_name=file.file,bucket= AWS_S3_BUCKET_NAME,key=uuid_string+"/"+file.filename )
                url_list.append("https://s3.amazonaws.com/"+AWS_S3_BUCKET_NAME+"/"+uuid_string+"/"+file.filename)
        ...
        if response:
            logging.info("Files uploaded on s3 bucket at the following path: %s",str(','.join(str(x) for x in url_list)))
            data={
            ...
            es.indices.create(index="product_info", ignore=400)
...
async def update_product(product_id:str,delete_list:List[str],add_items: List[UploadFile] = File(...) ,product:str= Form(...),description:str= Form(...)):
    try:
        res = es.search(index="product_info",body={'query': {'match' : {"_id":product_id}}})
        s3_list=res['hits']['hits'][0]['_source']['media_list']

        delete_list = [i.replace("https://s3.amazonaws.com/"+AWS_S3_BUCKET_NAME+"/","") for i in delete_list]
        ...
        if response:
            ...
            for file in add_items:
                response =s3_client.sync_upload_fileobj(file_name=file.file,bucket= AWS_S3_BUCKET_NAME,key=product_id+"/"+file.filename )
                s3_list.append("https://s3.amazonaws.com/"+AWS_S3_BUCKET_NAME+"/"+product_id+"/"+file.filename) 
            es.update_by_query(index="product_info",id=product_id, body={"doc":{"product_name":product,"description":description,"media_list":s3_list}})
