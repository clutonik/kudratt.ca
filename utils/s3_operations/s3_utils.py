...
class S3Events(object):
    ...
    def sync_upload_fileobj(self, file_name=None, bucket=None, key=None):
        ...
        :param key: Path where it has to be uploaded
        ...
        return True
    def sync_delete_folder(self,bucket_name,file_path):
        """
        Delete folder from s3 bucket
        :param bucket_name: Bucket from where to delete
        :param folder_prefix: Folder path in s3 bucket
        """
        s3_client = boto3.client('s3')
        try:
